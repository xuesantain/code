...
def _logprob_item_to_dict(token_id: int, info: Any, tokenizer: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "token": token,
        "logprob": float(getattr(info, "logprob")),
    }


def _extract_vllm_decode_rows(completion: Any, tokenizer: Any) -> list[dict[str, Any]]:
    token_ids = [int(token_id) for token_id in list(completion.token_ids)]
    ...
    for step, token_id in enumerate(token_ids):
        ...
        chosen_logprob = None if chosen_info is None else float(getattr(chosen_info, "logprob"))
        chosen_token = (
            _decode_one(tokenizer, token_id)
            if chosen_info is None
            else getattr(chosen_info, "decoded_token", None) or _decode_one(tokenizer, token_id)
        )
